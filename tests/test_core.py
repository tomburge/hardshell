...
import platform
import ctypes
from hardshell.utils.core import detect_admin
...
class TestDetectOs:
    ...
    def test_detect_os_returns_expected_dictionary(self):
        ...
        assert all(
            os_details[key] == value for key, value in zip(expected_keys, expected_values)
        )
    ...
    def test_detect_os_returns_expected_dictionary(self):
        ...
        assert all(key in os_details for key in expected_keys)
        assert all(
            value == os_details[key] for key, value in zip(expected_keys, expected_values)
        )
